# Wave-2 · 07 · Practical & strategic wisdom — **P2**

**Run as a fresh deep-research query in Claude chat. Save the report to `corpus/reports/wave-2/07-practical-strategic.md`.**
...
FOCUS:
- Strategy/conflict: Sun Tzu (Art of War — Giles 1910 is PD; note the fake-quote swamp);
  Miyamoto Musashi (Book of Five Rings); Kautilya (Arthashastra); the Melian dialogue
  as strategic realism (cross-reference: Thucydides-as-Nietzsche's-exemplar is already
  held — take the strategic angle only).
- Worldly conduct: Baltasar Gracián (The Art of Worldly Wisdom); Machiavelli (The
  Prince AND the Discourses — the republican Machiavelli, not just the meme); Lord
  Chesterfield only if the childhood/old-age query (running in parallel) hasn't taken
  him; Han Feizi only if the Eastern query hasn't — check and cross-reference.
...
- CARE: this stream is ethically spiky (manipulation, realpolitik). Represent it
  honestly and descriptively; mark claims cautionary/descriptive vs endorsed; keep
  claim types clean; SHOW the tension with the moral traditions rather than smoothing
  it — those tensions are first-class objects for this project.

ALREADY COVERED — DO NOT DUPLICATE: Machiavelli and Gracián are NOT yet held (despite
what older notes said) — treat them fully. Thucydides is held only as Nietzsche's
approved exemplar (realism, courage before reality) — flag overlap. La Rochefoucauld's
unmasking psychology and Bacon's worldly essays are held from a moralists report — the
cunning-vs-integrity fork should cross-reference them, not re-extract.
...
OUTPUT — end the report in these sections, in order:
  ...
  F) Misattribution watchlist (Sun Tzu and Machiavelli are heavily meme-quoted —
     verify against Giles and standard editions respectively).
